...
- `infra/docker`: Containerization setup (PostgreSQL, Redis, MinIO, Coturn, Backend).
...
MINIO_BUCKET_NAME=chatting-media
...
### 1. Boot Infrastructure (Postgres, Redis, MinIO, Coturn)
```bash
